...
class Layer1Service:
    ...
    def __init__(self, concurrency: int = CONCURRENCY):
        # Auth via environment variables (Railway) or ~/.netrc (local)
        ...
        if earthdata_username and earthdata_password:
            ...
            logger.info("Layer 1: Authenticated with NASA Earthdata (environment variables)")
        else:
            # Local development - use .netrc file
            earthaccess.login()
            logger.info("Layer 1: Authenticated with NASA Earthdata (.netrc)")
